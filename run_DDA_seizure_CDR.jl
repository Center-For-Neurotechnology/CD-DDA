...
# _norm01(A) = begin
#     mn = minimum(skipmissing(A[:]))
#     mx = maximum(skipmissing(A[:]))
#     mx == mn ? zero(A) : (A .- mn) ./ (mx - mn)
# end

# # --- finite-safe utilities (no normalization) ---
# _finite_minmax(A) = begin
#     vals = vec(A[isfinite.(A)])
#     isempty(vals) && return (0.0, 1.0)          # fallback if nothing finite
#     (minimum(vals), maximum(vals))
# end
...
#     Cw  = @view C[w, 1:n, 1:n]
#     Ew  = @view E[w, 1:n, 1:n]
#     CEw = Cw .* Ew  # raw, unnormalized

#     # Build display copy so NaNs don't blank the plot
#     Cw_disp  = copy(Cw);   Cw_disp[.!isfinite.(Cw_disp)] .= 0
#     Ew_disp  = copy(Ew);   Ew_disp[.!isfinite.(Ew_disp)] .= 0
#     CEw_disp = copy(CEw);  CEw_disp[.!isfinite.(CEw_disp)] .= 0

#     # Color limits (per-window by default; or pass global_clim=(lo,hi))
#     climC  = fix_clim ? _finite_minmax(Cw)  : nothing
#     climE  = fix_clim ? _finite_minmax(Ew)  : nothing
#     climCE = fix_clim ? (global_clim === nothing ? _finite_minmax(CEw) : global_clim) : nothing
#     # guard equal limits
#     climC  = (climC[1]  == climC[2])  ? (climC[1]-1e-9,  climC[2]+1e-9)  : climC
#     climE  = (climE[1]  == climE[2])  ? (climE[1]-1e-9,  climE[2]+1e-9)  : climE
#     climCE = (climCE[1] == climCE[2]) ? (climCE[1]-1e-9, climCE[2]+1e-9) : climCE
...
# Loads `C` and `E` ONCE from `FN_ALL`, saves plots for requested windows, then frees memory.
# If `windows` is `nothing`, saves for all windows.
# """
# function save_all_windows_from_file(FN_ALL::AbstractString; 
#                                     windows=nothing,
#                                     out_dir::String="FIG", 
...
# # plot_C_E_CE_from_file(FN_ALL; w=10, out_dir=FIG_DIR, prefix=base*"_C_E", save=false)
# save_all_windows_from_file(FN_ALL; out_dir=FIG_DIR, prefix=base*"_C_E")